...
test("drag and drop moves a task to another column", () => {
  ...
  expect(screen.getByText(/todo/i).parentElement).not.toHaveTextContent("Task 1");
});
...
test("dropping outside valid list does not break anything", () => {
  ...
  expect(getColumns().todo).toHaveTextContent("Task 1");
});
...
test("dragging multiple items and dropping them reorders correctly", () => {
  ...
  expect(getColumns().todo).not.toHaveTextContent("Task 1");
  expect(getColumns().todo).not.toHaveTextContent("Task 2");
});
